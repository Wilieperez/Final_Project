...
void *receive_frame(void *param){
   ...
   while((numbytes=recv(fd,(char *)&pub_frame,sizeof(f_Publish),0)) > 0){
      ...
      }else if(pub_frame.bFrameType == 0xB0){
         unsubAck_frame.bFrameType = pub_frame.bFrameType;
         unsubAck_frame.bRemainLen = pub_frame.bRemainLen;
         unsubAck_frame.bResponse = pub_frame.bTopic;
         if(unsubAck_frame.bResponse == 0x01){
            printf("You aren't subscribed to that topic\n");
         }else if(unsubAck_frame.bResponse == 0x00){
...
int main(int argc, char *argv[])
{
   ...
   while(option != 3){
         ...
         case 1:
            ...
            printf("========================\n");
            printf("0. Temperature\n1. Rain Chance\n2. Humidity\n3. UV Index\n4. Air Quality\n");
            printf("========================\n- ");
            scanf("%i", &topicNum);

            if(topicNum != 0 && topicNum != 1 && topicNum != 2 && topicNum != 3 && topicNum != 4){
         ...
         case 2:
            ...
            if(topicNum != 0 && topicNum != 1 && topicNum != 2 && topicNum != 3 && topicNum != 4){
               ...
               if(pthread_mutex_lock(&mutex) == 0){
                  unsub_frame = create_Unsub(topicNum);//Create unsubscribe frame with chosen topic
                  send(fd,(char *)&unsub_frame,sizeof(f_Unsubscribe),0);
                  pthread_mutex_unlock(&mutex);
               }
               break;
            }
